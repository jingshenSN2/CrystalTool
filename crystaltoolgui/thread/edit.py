...
class EditThread(threading.Thread):
    def __init__(self, hkl_files, method, edit_range, params, is_scale, scale_min, scale_max, signal):
    ...
    def run(self):
        ...
        for hkl_file in self.hkl_files:
            process += 1
            output = edit_hkl(hkl_file, self.method, self.edit_range, self.params,
                              self.is_scale, self.scale_min, self.scale_max)
